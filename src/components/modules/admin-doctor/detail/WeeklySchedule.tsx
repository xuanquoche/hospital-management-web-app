    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <div className="flex items-center gap-4">
          ...
          <Button variant="outline" size="sm" className="bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100 hover:text-emerald-800 dark:bg-emerald-950/30 dark:text-emerald-400 dark:border-emerald-800">
      ...
      <CardContent className="pt-6">
                  ...
                  {day.slots.map((slot, slotIndex) => (
                    <div
                      ...
                      className={cn(
                        ...
                        slot.type === 'In-person' ? "bg-emerald-600" : "bg-emerald-600" // Using same color for now based on image, can differentiate if needed
                      )}
                    >
                      <div>{slot.start} - {slot.end} · {slot.type}</div>
                    </div>
                  ))}
                </div>
              )}
        ...
        <div className="mt-6 text-xs text-muted-foreground">
          Use "Khoảng ngày tùy chỉnh" to define custom date ranges and time slots for this doctor. Existing appointments remain unchanged.
        </div>
      </CardContent>
    </Card>
  );
