  ...
  return (
    <div className='bg-white rounded-2xl p-6 shadow-sm border border-slate-100'>
      <div className='flex justify-between items-start mb-6'>
        <div>
          <h3 className='text-lg font-bold text-slate-900'>Chi tiết hóa đơn được chọn</h3>
      ...
      <div className='bg-slate-50 rounded-lg p-3 mb-6 flex flex-wrap gap-4 items-center text-sm'>
        <span className='font-semibold text-teal-700'>Mã hóa đơn: {invoice.id}</span>
        <span className='text-slate-400'>|</span>
        <span className='text-slate-600'>Thanh toán trước: {invoice.dueDate?.split('-').reverse().join('/')}</span>
      </div>
      ...
      <div className='border-t border-slate-100 pt-4 space-y-3 mb-6'>
        <div className='flex justify-between items-center text-sm'>
          <span className='font-semibold text-slate-700'>Tổng tiền dịch vụ</span>
          <span className='font-bold text-slate-900'>
            {new Intl.NumberFormat('vi-VN', {
              style: 'currency',
              currency: 'VND',
        ...
        <div className='flex justify-between items-center text-sm'>
          <span className='font-semibold text-slate-700'>Giảm giá / Bảo hiểm</span>
      ...
      <p className='text-xs text-slate-500 mb-4'>
        Lưu ý: Sau khi thanh toán thành công, hóa đơn sẽ được chuyển sang mục "Đã thanh toán" và gửi về email của bạn.
      </p>

      <div className='flex justify-end items-center gap-4'>
        <span className='text-sm font-medium text-slate-600'>Thanh toán sau</span>
