...
export const SpecialtyList = () => {
  return (
    <div className='rounded-2xl border border-slate-100 bg-white p-6 shadow-sm'>
      <div className='mb-4'>
        <h3 className='text-lg font-bold text-slate-900'>Chọn theo chuyên khoa</h3>
        <p className='text-slate-500'>Hoặc chọn chuyên khoa trước, sau đó chọn bác sĩ.</p>
      </div>
      ...
      <div className='space-y-3'>
        {specialties.map((specialty) => (
          ...
          >
            <h4 className={`font-bold ${specialty.active ? 'text-teal-800' : 'text-slate-700'}`}>{specialty.name}</h4>
            <p className={`text-sm ${specialty.active ? 'text-teal-600' : 'text-slate-500'}`}>
