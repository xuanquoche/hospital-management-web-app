  ...
  return (
    <div className='bg-white rounded-2xl p-6 shadow-sm border border-slate-100'>
      <div className='flex justify-between items-center mb-6'>
        <h3 className='text-lg font-bold text-slate-900'>Hình thức thanh toán</h3>
        <span className='text-xs font-medium text-teal-600 bg-teal-50 px-2 py-1 rounded-full'>Thanh toán an toàn</span>
      </div>

      <p className='text-sm text-slate-500 mb-6'>Chọn một trong các phương thức sau.</p>
        ...
        {methods.map((method) => (
          ...
          >
            <div
              className={cn(
                'p-3 rounded-lg mr-4',
                paymentMethod === method.id ? 'bg-teal-100 text-teal-600' : 'bg-slate-100 text-slate-500'
              )}
            >
            ...
            <div className='flex-1'>
              <h4 className={cn('text-sm font-bold', paymentMethod === method.id ? 'text-teal-900' : 'text-slate-900')}>
                {method.title}
              </h4>
            ...
            <div
              className={cn(
                ...
                paymentMethod === method.id ? 'border-teal-500' : 'border-slate-300'
              )}
            >
              {paymentMethod === method.id && <div className='w-2.5 h-2.5 rounded-full bg-teal-500' />}
            </div>
          </div>
        ))}
      </div>
