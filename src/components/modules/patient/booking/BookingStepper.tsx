  ...
  return (
    <div className='w-full rounded-2xl border border-slate-100 bg-white p-6 shadow-sm'>
      <div className='mb-4'>
        ...
        <p className='text-slate-500'>4 bước đơn giản để hoàn tất đặt lịch khám.</p>
      </div>
      ...
      <div className='relative flex items-center justify-between'>
        ...
        {steps.map((step) => {
          ...
          return (
            <div key={step.id} className='relative z-10 flex items-center gap-2 bg-white px-2'>
              <motion.div
                initial={false}
                animate={{
                  backgroundColor: isActive || isCompleted ? '#0d9488' : '#f1f5f9',
              ...
              <span className={`hidden text-sm font-medium sm:block ${isActive ? 'text-teal-700' : 'text-slate-500'}`}>
                {step.label}
              </span>
            </div>
          );
