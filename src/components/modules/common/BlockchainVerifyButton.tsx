...
import { useBlockchainVerify } from '@/hooks/use-blockchain-verify';
import {
...
export const BlockchainVerifyButton = ({
  ...
  const { isLoading, error, verifyResult, verify, reset } =
    useBlockchainVerify();
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      if (record && dataHash) {
        await verify(documentId, dataHash);
      }
    } finally {
      setIsFetching(false);
    }
  ...
  const renderStatus = () => {
      ...
      return (
        <div className='flex flex-col items-center justify-center py-8'>
          ...
          <p className='text-sm text-slate-500 text-center'>
            Tài liệu này chưa được ghi lên blockchain. Liên hệ bác sĩ hoặc admin
            để yêu cầu ghi blockchain.
          </p>
        </div>
      );
    }
    ...
    if (verifyResult) {
      if (verifyResult.isRevoked) {
        return (
          <div className='flex flex-col items-center justify-center py-8'>
            ...
            <p className='text-sm text-slate-500 text-center'>
              Tài liệu này đã bị thu hồi và không còn hiệu lực.
            </p>
          </div>
        );
      }
        ...
        return (
          <div className='flex flex-col items-center justify-center py-8'>
            ...
            <p className='text-sm text-slate-500 text-center mb-4'>
              Tài liệu hợp lệ và chưa bị chỉnh sửa kể từ khi ghi blockchain.
            </p>
      ...
      return (
        <div className='flex flex-col items-center justify-center py-8'>
          ...
          <p className='text-sm text-slate-500 text-center'>
            Tài liệu có thể đã bị chỉnh sửa sau khi ghi blockchain. Vui lòng
    ...
    return (
      <div className='flex flex-col items-center justify-center py-8'>
        ...
        <p className='text-sm text-slate-500 text-center'>
          Tài liệu này đã được ghi lên blockchain.
        </p>
      </div>
    );
  };
              ...
              <div className='space-y-2 text-sm'>
                ...
                <div className='flex justify-between items-center'>
                  ...
                  <code className='text-xs bg-slate-100 px-2 py-1 rounded max-w-[200px] truncate'>
                    {blockchainRecord.dataHash.slice(0, 20)}...
                  </code>
                </div>
                {txHash && (
                  <div className='pt-2'>
