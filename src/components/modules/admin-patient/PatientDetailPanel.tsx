          ...
          <div className="space-y-1">
            ...
            <div className="flex flex-wrap gap-2">
              {patient.tags.map((tag) => (
                <Badge key={tag} variant="secondary" className="bg-slate-100 text-slate-700 hover:bg-slate-100/80 text-[10px] px-1.5 py-0 h-5">
                  {tag}
                </Badge>
              ))}
            </div>
            ...
            {patient.visitHistory.map((visit, index) => (
              <div key={index} className="flex items-start justify-between">
                <div className="space-y-0.5">
                  <p className="text-xs font-medium">{visit.date} · {visit.type}</p>
                  <p className="text-muted-foreground text-[10px]">Follow-up · {visit.doctor}</p>
                </div>
                <Badge variant="secondary" className="bg-slate-100 text-slate-500 hover:bg-slate-100/80 text-[10px] px-1.5 py-0 h-5 font-normal">
                  {visit.status}
                </Badge>
              </div>
            ))}
