...
import { ROUTES, PUBLIC_ROUTES, PRIVATE_ROUTES, LOCALIZED_ROUTES } from './const/routes';
...
export async function middleware(request: NextRequest) {
  ...
  const isPublicRoute = publicRouteValues.some((route) => pathname.startsWith(route));

  // Kiểm tra private route (cả có và không có locale)
  const isPrivateRoute = privateRouteValues.some(
    (route) => pathname.startsWith(route) || pathname.startsWith('/en' + route) || pathname.startsWith('/ja' + route)
  );
  ...
  if (isPublicRoute) {
    if (isAuthenticated) {
      ...
      const locale = request.nextUrl.searchParams.get('locale') || pathname.startsWith('/ja') ? 'ja' : 'en';
      return NextResponse.redirect(new URL(LOCALIZED_ROUTES.DASHBOARD(locale), request.url));
    }
  ...
  // --- Xử lý root '/' ---
  if (pathname === '/') {
    const locale = request.headers.get('accept-language')?.split(',')?.[0]?.split('-')?.[0] === 'ja' ? 'ja' : 'en';
